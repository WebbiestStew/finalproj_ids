...
// Safe to re-run: it only inserts what is missing, and never touches real data.
...
const alreadySeeded = db.prepare('SELECT COUNT(*) AS n FROM vehicles WHERE dealer_id IN (?, ?)').get(...dealerIds).n;
if (alreadySeeded > 0) {
  console.log(`Ya hay ${alreadySeeded} vehículos demo; no se agregó nada.`);
} else {
  ...
  console.log(`Se agregaron ${CARS.length} vehículos demo.`);
}
console.log(`Concesionarias demo: ${DEALERS.map((d) => d.email).join(', ')} (contraseña: ${DEMO_PASSWORD})`);
